...
class SelfAttentionLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, key_padding_mask: Optional[torch.Tensor] = None, attn_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # # Ensure the attention mask is properly shaped
        # # Ensure attn_mask has the correct shape
        # if attn_mask is not None:
        #     # Convert attn_mask to a shape expected by nn.MultiheadAttention
        #     # MultiheadAttention expects attn_mask of shape (seq_len, seq_len)
        #     if attn_mask.dim() == 2 and attn_mask.shape[0] != x.shape[1]:
