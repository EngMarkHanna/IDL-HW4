...
class ASRDataset(Dataset):
    def __init__(
        ...
        if self.partition != "test-clean":
            ...
            self.text_dir   = self.text_dir = os.path.join(config['root'], partition, 'text')
        ...
        for i in tqdm(range(self.length)):
            ...
            if self.partition != "test-clean":
                ...
                tokenized = self.tokenizer.encode(  "".join(transcript))
    ...
    def __getitem__(self, idx) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        """
        # TODO: Load features
        feat = self.feats[idx]  # originally (num_feats, time)
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        batch_feats = [item[0].T for item in batch]  # Transpose to (time, feat) for batching
        ...
        # batch_feats  = [torch.tensor(sample['features'], dtype=torch.float32) for sample in batch]
        ...
        # feat_lengths = torch.tensor([sample['feat_length'] for sample in batch], dtype=torch.long) # B
        ...
        padded_feats = pad_sequence(batch_feats, batch_first=True, padding_value=self.tokenizer.pad_id) # B x T x F
        ...
        if self.partition != "test-clean":
            ...
            # batch_shifted = [torch.tensor(sample[1], dtype=torch.long) for sample in batch]# B x T
            # batch_golden = [torch.tensor(sample[2], dtype=torch.long) for sample in batch]# B x T
